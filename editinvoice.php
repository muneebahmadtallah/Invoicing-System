...
/////////////////////
$slctqt= mysqli_query($con,"SELECT * FROM invoice_products where id = '$invoice_products_ids'");
$da= mysqli_fetch_array($slctqt);
$recent_qauntity=$da["Quantity"];
$pid=$da["product_id"];
// echo $recent_qauntity. "recent<br>";
// echo $Quantity."qty<br>";
// echo $pid."product id<br>";
if($recent_qauntity > $Quantity)
{
$sum =  $recent_qauntity - $Quantity;
// echo $sum."sum if one <br>";
$qty= "UPDATE products set quantity = (quantity + '$sum') WHERE id = '$pid'";
$result=mysqli_query($con,$qty);
}
else
{
$sum =  $Quantity - $recent_qauntity ;
// echo $sum."sum else one <br>";
  $qty2= "UPDATE products set quantity = (quantity - '$sum') WHERE id = '$pid'";
$result2=mysqli_query($con,$qty2);
}
...
<?php  
 $count =0;
 $slct2 = "SELECT invoice_products.id, invoice_products.invoice_id, invoice_products.product_id, invoice_products.ItemPrice,
 ...
 if(mysqli_num_rows($result2) > 0)
 {
  ...
  <tr>
    ...
    <td><button class="btn btn-danger remove"><a class="text-white" href="deleteinvoiceproduct.php?id=<?php echo $data2['id']; ?>" 
   >Delete</a></button></td>
